...
function appendJSConfig(data) {
    ...
    var newJsc, mergedJsc, currentJsc;
...
function appendVSCIgnore(ignoreItems) {
    ...
    var eol = os_1.EOL || (process.platform === "win32" ? "\r\n" : "\n");
...
function hasJSConfig() {
    return hasFile("jsconfig.json");
}
exports.hasJSConfig = hasJSConfig;
function hasVCSIgnore() {
    return hasFile(".vscodeignore");
}
exports.hasVCSIgnore = hasVCSIgnore;
function hasFile(file) {
    if (!vscode_1.workspace || !vscode_1.workspace.rootPath) {
        return false;
    }
    return pathExists.sync(path.join(vscode_1.workspace.rootPath, file));
}
