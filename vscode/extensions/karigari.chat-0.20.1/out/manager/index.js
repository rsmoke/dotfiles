...
const vslsCommunities_1 = require("../vslsCommunities");
class Manager {
    constructor(store) {
        this.store = store;
        ...
        this.initializeVslsContactProvider = async () => {
            ...
            const nonVslsProviders = enabledProviders.filter(provider => provider !== "vsls" && provider !== "vslsCommunities");
    ...
    getEnabledProviders(newInitialState) {
        ...
        const hasVslsCommunities = utils_1.hasVslsCommunitiesExtension();
        if (hasVslsCommunities) {
            providerTeamIds["vslsCommunities" /* vslsCommunities */] = undefined;
        }
        if (!!newInitialState) {
    ...
    instantiateChatProvider(token, provider) {
        switch (provider) {
            ...
            case "vslsCommunities":
                return new vslsCommunities_1.VslsCommunitiesProvider();
            default:
