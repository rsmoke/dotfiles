...
class ExtensionUtil {
    ...
    static getElectronPath() {
        ...
        switch (os.platform()) {
            case 'darwin':
                electronPath = path.join(ExtensionUtil.getGlobalStoragePath(), `/${packageCode}/dist`, 'Electron.app/Contents/MacOS/Electron');
                break;
            case 'freebsd':
            case 'linux':
                electronPath = path.join(ExtensionUtil.getGlobalStoragePath(), `/${packageCode}/dist`, 'electron');
                break;
            case 'win32':
            default:
                electronPath = path.join(ExtensionUtil.getGlobalStoragePath(), `/${packageCode}/dist`, 'electron.exe');
                break;
        }
